...
@section('footer-scripts')
    ...
    $(document).ready(function () {
        $('#pEggId').select2({placeholder: 'Select a Nest Egg'}).on('change', function () {
            ...
            var parentChain = _.get(Ibra Decode.nests, $("#pNestId").val());
            ...
            for (let i = 0; i < keys.length; i++) {
                ...
                if (objectChain.id === parseInt(Ibra Decode.server.egg_id) && Ibra Decode.server.image == opt.value) {
                    opt.selected = true
                }
            ...
            if (objectChain.id === parseInt(Ibra Decode.server.egg_id)) {
                if ($('#pDockerImage').val() != Ibra Decode.server.image) {
                    $('#pDockerImageCustom').val(Ibra Decode.server.image);
                }
            }
            ...
            $.each(_.get(objectChain, 'variables', []), function (i, item) {
                var setValue = _.get(Ibra Decode.server_variables, item.env_variable, item.default_value);
        ...
        $('#pNestId').select2({placeholder: 'Select a Nest'}).on('change', function () {
            $('#pEggId').html('').select2({
                data: $.map(_.get(Ibra Decode.nests, $(this).val() + '.eggs', []), function (item) {
                    return {
                        id: item.id,
                        text: item.name,
                    };
            ...
            if (_.isObject(_.get(Ibra Decode.nests, $(this).val() + '.eggs.' + Ibra Decode.server.egg_id))) {
                $('#pEggId').val(Ibra Decode.server.egg_id);
            }
